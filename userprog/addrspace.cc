...
// how big is address space?
    ...
    ASSERT(numPages <= (unsigned int) gPhysPageBitMap->NumClear()); // changed here
...
// first, set up the translation
    ...
    for (i = 0; i < numPages; i++) {
        ...
        pageTable[i].physicalPage = gPhysPageBitMap->Find(); // changed here
    ...
    // Read the segmented pages in pageTable for application to run
    unsigned int tempPageSize, firstPage, firstPageSize, lastPage, lastPageSize, inFileAddr;
    ...
    if (noffH.code.size > 0) {
        ...
        if (tempPageSize < (unsigned int) noffH.code.size) {
    ...
    if (noffH.initData.size > 0) {
        ...
        if (tempPageSize < (unsigned int) noffH.initData.size) {
            firstPageSize = tempPageSize;
